...
账本默认打开「本月」，每条记录都归属到某个月。

1. **记一笔**：点右上角「记一笔」（手机在底栏中间，桌面也可以直接按 `N`），
   名称填「吃」、金额 `960`、备注「（5+15）*30」，保存。
   → 顶部的本月支出变成 `¥960.00`，近 12 个月的柱子里本月那根亮起来。
2. **放进分组**：再记一笔「地铁 120」，分组点「新分组」填「交通」；
   接着记「单车 25」「公交 60」，分组直接点已经出现的「交通」。
   → 列表里「交通」显示 `205.00`，点它展开能看到三项；分组金额不可手填。
3. **改 / 删**：点「单车」那一行，把金额改成 `30` 保存，合计立即重算；
   再点开它点「删除」，底部提示里点「撤销」，记录原样回来。
4. **从上月复制**：点月份标题右边的 `›`（或按 `→`）翻到下个月，空月份会提示「从 X 月复制」，点一下。
   → 上个月的条目和分组全部出现；再从「⋯」菜单里复制一次（会先让你确认），会提示已经都有了，不会重复生成。
5. **批量生成**：「⋯ → 批量生成到多个月」，分组填「房租」，条目「主卧 3200」，月份选到 12 月，生成。
   → 每个月各一条；再生成一次会全部跳过（防重复）。
6. **搜索**：点放大镜（或按 `/`）搜「地铁」，结果按月分组，顶部是合计；点月份名跳到那个月。
7. **看统计**：切到「统计」，换几个时间范围、切「按月 / 按年」，
   看四个数字、柱状图、「花在哪里」是否跟着变；点某个月的柱子会跳回那个月的账本。
